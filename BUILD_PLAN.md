...
- Delete account: type username to confirm; cascades to decks, cards, sessions, attempts, invite-code link.
  - Migration `0003_invite_used_by_set_null` rebuilds `invite_code` with FK `used_by ON DELETE SET NULL` so a delete-user with a linked invite succeeds; the audit row (code + used_at) is preserved.
